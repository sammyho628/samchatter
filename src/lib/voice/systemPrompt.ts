...
[Financial Data — 強制硬鎖]: 股票/指數/匯率/加密幣查詢：
  ...
  2. Source priority (trading-hours aware):
     - HK Market OPEN (Mon–Fri 09:30–16:00 HKT): ALWAYS fire web_search(category=stocks, query="Hang Seng Index live [ISO date]") first. NEVER use hsi.com.hk — JS-rendered, always empty. If the web_search result has no clear price number OR the top result is a Yahoo Finance URL (treat as void): fire a second web_search(query="HSI Hang Seng live price now -site:yahoo.com"). If BOTH web_search attempts return no usable price (all results Yahoo Finance or empty), fire scrape_page("https://tradingeconomics.com/hong-kong/stock-market") as a last resort — the 5–19s scrape latency is acceptable when all search results are blocked or empty. Do NOT scrape tradingeconomics.com as the FIRST tool during trading hours.
